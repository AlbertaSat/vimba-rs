...
mod vimba_api {
    ...
    pub fn connect_camera() -> vimba_rs::VmbResult<vimba_rs::api::CameraHandle> {
        ...
        let first_camera = cameras
            .iter()
            .find(|camera| camera.permitted_access.access_full())
            .or_else(|| cameras.first())
            .ok_or(vimba_rs::VmbError::NotFound)?; // ok_or transforms Some(v) into Ok(v) and None into Err(e), where e is arg to ok or (here is VmbError::NotFound)
        ...
        */
        ...
        let camera_handle =
            vimba_rs::api::camera_open(&first_camera.id, vimba_rs::api::AccessMode::Full)?;

        println!("Camera handle: {:?}", camera_handle.as_raw());
    ...
    unsafe extern "C" fn frame_done_callback(
        ...
        println!(
            ...
            frame_ref.frameID, frame_ref.width, frame_ref.height
        );
    ...
    pub fn capture_asynchronous(
        camera_handle: &impl vimba_rs::api::VmbHandle,
        n: u32,
    ) -> vimba_rs::VmbResult<()> {
        ...
        for frame in &frames {
            vimba_rs::api::capture_frame_queue(camera_handle, frame, None)?; //register the callback, which must be of type VmbFrameCallback*()
        }
        //start image acquisition:
        // run camera command feature AcquisitionStart
        println!("Feature info query for AcquisitionStart:");
        vimba_rs::api::feature_info_query(camera_handle, "AcquisitionStart")?;

        println!("Running AcquisitionStart");
        vimba_rs::api::feature_command_run(camera_handle, "AcquisitionStart")?;

        println!("Capturing frmaes");
        ...
        println!("Cleaning up after acuiqition");
    ...
    pub fn write_feature(
        ...
        match get_feature_info(camera_handle, feature_str_name) {
                    ...
                    true => {
                        match feature_info.data_type {
                            ...
                            vimba_rs::api::FeatureDataType::Integer => {
                                ...
                                let value_to_write_converted = value_to_write
                                    .parse::<i64>()
                                    .map_err(|_| vimba_rs::VmbError::BadParameter)?;
                                ...
                                vimba_rs::api::feature_int_set(
                                    camera_handle,
                                    feature_str_name,
                                    value_to_write_converted,
                                ) //returns VmbResult<()>
                            }
                            vimba_rs::api::FeatureDataType::Float => {
                                ...
                                let value_to_write_converted = value_to_write
                                    ...
                                    .map_err(|_| vimba_rs::VmbError::BadParameter)?;
                                println!("Converted value: {}", value_to_write_converted);
                                vimba_rs::api::feature_float_set(
                                    camera_handle,
                                    feature_str_name,
                                    value_to_write_converted,
                                )
                            ...
                            vimba_rs::api::FeatureDataType::String => {
                                println!("Feature data type: {:?}", feature_info.data_type);
                                vimba_rs::api::feature_string_set(
                                    camera_handle,
                                    feature_str_name,
                                    value_to_write,
                                ) //returns VmbResult<()>
                            ...
                            vimba_rs::api::FeatureDataType::Command => {
                                println!("Feature data type: {:?}", feature_info.data_type);
                                println!(
                                    "write_feature is not a method for this feature data type."
                                );
            ...
            Err(e) => {
                println!(
                    "Error getting feature info for {}. Error: {e}",
                    feature_str_name
                );
                Err(e)
            }
    ...
    pub fn read_feature(
        camera_handle: &impl vimba_rs::api::VmbHandle,
        feature_str_name: &str,
    ) -> vimba_rs::VmbResult<vimba_rs::api::FeatureValue> {
        ...
        if let Some((feature_name, enum_entry_name)) = feature_str_name.split_once('.') {
            ...
            let v = vimba_rs::api::feature_enum_entry_get(
                camera_handle,
                feature_name,
                enum_entry_name,
            )?;
                ...
                Ok(feature_info) => {
                    ...
                    match feature_info.flags.read_access() {
                        true => {
                            match feature_info.data_type {
                                vimba_rs::api::FeatureDataType::Integer => {
                                    let v = vimba_rs::api::feature_int_get(
                                        camera_handle,
                                        feature_str_name,
                                    )?;
                                ...
                                vimba_rs::api::FeatureDataType::Float => {
                                    let v = vimba_rs::api::feature_float_get(
                                        camera_handle,
                                        feature_str_name,
                                    )?;
                                ...
                                vimba_rs::api::FeatureDataType::Bool => {
                                    let v = vimba_rs::api::feature_bool_get(
                                        camera_handle,
                                        feature_str_name,
                                    )?;
                                ...
                                vimba_rs::api::FeatureDataType::String => {
                                    let v = vimba_rs::api::feature_string_get(
                                        camera_handle,
                                        feature_str_name,
                                    )?;
                                ...
                                vimba_rs::api::FeatureDataType::Enum => {
                                    let v = vimba_rs::api::feature_enum_get(
                                        camera_handle,
                                        feature_str_name,
                                    )?;
...
fn main() {
    vimba_api::initialize();
    {
        ...
        match vimba_api::connect_camera() {
            Ok(camera_handle) => {
                println!(
                    "Successfully connected to camera with handle: {:?}",
                    &camera_handle
                );
                ...
                match vimba_rs::api::list_features(&device_handle) {
                    ...
                    Err(e) => {
                        eprintln!("Failed to list device features: {e}");
                    }
                }
